...
export default props => {
    ...
    const headerItems = data.settings.frontmatter.ordering.map(pageTitle =>
        data.pages.nodes.find(node => node.frontmatter.title === pageTitle),
    )

    return (
        <nav className="flex justify-between flex-wrap bg-gray-700 px-6 fixed w-full z-10 top-0">
            {/*Website title*/}
            <div className="block flex-shrink-0 mr-6 px-4 py-2 text-2xl text-white no-underline hover:text-white">
                <Link className="header-link-home" to="/">
                    {data.settings.frontmatter.title}
                </Link>
            </div>
            ...
            <div className="block lg:hidden py-3" onClick={onMenuClick}>
                ...
                >
                    <svg
                        className="fill-current h-8 w-3"
                        viewBox="0 0 20 20"
